...
- [API Versioning](./api-versioning.md) - public versioning config surface, route registration, the `AppBootstrapConfig` handoff boundary, and runtime boundaries.
- [Server Core](./server-core.md) - transport shell, app-facing bootstrap config boundary, and current TLS/runtime boundaries, with the SCXML request-pipeline gate kept explicit.
