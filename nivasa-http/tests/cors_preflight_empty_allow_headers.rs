...
use nivasa_http::{CorsOptions, NivasaServer};
...
#[tokio::test]
async fn cors_preflight_omits_allow_headers_when_configured_list_is_empty(
) -> Result<(), Box<dyn Error>> {
    let port = free_port();
    let (shutdown_tx, shutdown_rx) = oneshot::channel();

    let server = NivasaServer::builder()
        .cors_options(CorsOptions::permissive().allow_headers(Vec::<&str>::new()))
    ...
    let request = http::Request::builder()
        ...
        .header(ACCESS_CONTROL_REQUEST_HEADERS, "x-api-version")
